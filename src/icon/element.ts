...
import { IconService, Directions, isNumericString, BroadcastSubscription } from '../internals/index.js';
...
export class BpIcon extends LitElement {
  ...
  @property({ type: String, reflect: false }) type: '' | 'solid';
  ...
  get #iconSvg() {
    ...
    return `${icon.type[!!this.type ? this.type : 'default'] ?? icon.type.default}${this.badge || this.badge === '' ? '<circle cx="30" cy="4" r="4" class="badge"></circle>' : ''}`;
  }

  render() {
    return typeof IconService.registry[this.shape]?.shape === 'string'
      ? html`<span .innerHTML=${IconService.registry[this.shape]?.shape as string}></span>`
      : svg`<svg .innerHTML=${this.#iconSvg} viewBox="0 0 36 36" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"></svg>`;
  }
