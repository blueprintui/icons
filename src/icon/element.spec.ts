...
describe('icon element', () => {
  ...
  it('should support numeric values when setting size property', async () => {
    element.size = '36';
    await elementIsStable(element);
    expect(getComputedStyle(element).getPropertyValue('--width')).toBe('36px');
    expect(getComputedStyle(element).getPropertyValue('--height')).toBe('36px');

    element.size = '24';
    await elementIsStable(element);
    expect(getComputedStyle(element).getPropertyValue('--width')).toBe('24px');
    expect(getComputedStyle(element).getPropertyValue('--height')).toBe('24px');
  });
  ...
  it('should support t-shirt values when setting size property', async () => {
    ...
    element.size = 'lg';
    await elementIsStable(element);
    expect(getComputedStyle(element).getPropertyValue('--width').trim()).toBe('calc(1 * 48px)');
    expect(getComputedStyle(element).getPropertyValue('--height').trim()).toBe('calc(1 * 48px)');
  });
});
